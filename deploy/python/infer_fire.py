...
def queue_img_put(q_put,rtmp_str):
...
def paddle_model(FLAGS,origin_img_q, result_img_q):
    ...
    while is_opened:
        ...
        while True:
            ...
            # img = np.expand_dims(origin_img,axis = 0)
            
            print(origin_img.shape)
            results = detector.predict(origin_img, FLAGS.threshold)
            ## img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            ## cv2.imshow('video', origin_img)
            ## cv2.waitKey(1)
            
            result_img_q.put((origin_img, results,detector.config.labels,detector.config.mask_resolution))
           

# Draw the results of the detection (aka 'visulaize the results')
def queue_img_get(q_get,show_img_q):
    print("show result")
    
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter('fire_result.avi', fourcc, 20.0, (1024,576)) #1024.576
    ...
    try:
        while(True):
            ...
            (origin_img, results, labels, mask_resolution) = q_get.get()
            im = visualize_box_mask(
            origin_img,
            results,
            labels,
...
if __name__ == '__main__':
    ...
    turn_q = mp.Queue(maxsize=1)
    
    processes = [
        mp.Process(target=queue_img_put, args=(origin_img_q, rtmp_str)),
        mp.Process(target=paddle_model, args=(FLAGS,origin_img_q, result_img_q)),
        mp.Process(target=queue_img_get, args=(result_img_q,turn_q)),
